...
module.exports = function profile(url, html, next) {
  ...
  if (related && related.children && related.children.length > 0) {
    for (i = 0; i < related.children.length; i++) {
      var children = related.children[i];

      var relatedurl = children.children.find(function(e) {return e.name === 'a'})['attribs'].href;

      children = children.children.find(function(e) {return e.name === 'div'});
      var relatedheadline = children.children.find(function(e) {return e.name === 'p'}).children[0].data;

      data.related.push({url: buildProfileUrl(relatedurl), headline: relatedheadline});
    }
  }
