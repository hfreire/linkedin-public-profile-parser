...
module.exports = function profile(url, html, next) {
  ...
  if (related && related.children && related.children.length > 0) {
    for (i = 0; i < related.children.length; i++) {
      var el = related.children[i];
      var relatedurl = _.find(el.children, {name: 'a'})['attribs'].href;
      var relatedheadline = _.find(_.find(el.children, {name: 'div'}).children, {name: 'p'}).children[0].data;
