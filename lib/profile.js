...
module.exports = function profile(url, html, next) {
  ...
  data.canonicalurl = $('link[rel^="canonical"]')[0]['attribs'].href;
  ...
  if(langs.length > 0) {
    ...
    for(i = 0; i < langs.length; i++) {
      var lang = langs[i].children[0].children[0].children[0].data.trim();
  ...
  if(positions.length > 0) {
    for(i = 1; i < positions.length; i++) {
      ...
      exp.org = $('.position:nth-child('+i+') h5.item-subtitle').text().trim();
  ...
  var related = $('.insights.profile-section .browse-map ul')[0];
  data.related = [];
  if (related && related.children && related.children.length > 0) {
    for (i = 0; i < related.children.length; i++) {
      var cursor = related.children[i];

      var relatedurl = cursor.children.find(function(e) {return e.name === 'a'})['attribs'].href;

      cursor = cursor.children.find(function(e) {return e.name === 'div'});
      var headlineTag = cursor.children.find(function(e) {return e.name === 'p'});

      var relatedheadline = headlineTag && headlineTag.children[0].data;

      if (relatedurl && relatedheadline) {
        data.related.push({url: relatedurl.split('?')[0], headline: relatedheadline});
      }
    }
  }
